...
uses this information to construct a `m.room.member` event which it
...
and replaces the `event_id` with it's own. This is then sent to the
...
will also send a `m.room.third_party_invite` event in the room to
...
The invited homeserver will create a `m.room.member` invite event
...
Read receipts for events events that a user sent do not need to be sent.
